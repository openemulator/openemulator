		...
		node = node->next)
	{
		if (!xmlStrcmp(node->name, BAD_CAST "property"))
		{
			...
			if (hasNodeProperty(node, "value"))
			{
				...
				value = parseXMLProperties(value, propertiesMap);
			...
			else if (hasNodeProperty(node, "src"))
			{
				...
				string parsedSrc = parseXMLProperties(src, propertiesMap);
				bool dataRead = false;
				if (hasXMLProperty(src, "packagePath"))
				{
					if (package)
		...
		node = node->next)
	{
		if (!xmlStrcmp(node->name, BAD_CAST "property"))
		{
			...
			else if (hasNodeProperty(node, "src"))
			{
				...
				if (!hasXMLProperty(src, "packagePath") || !package)
					continue;
...
bool OEEmulation::hasXMLProperty(string value, string propertyName)
{
	return (value.find("${" + propertyName + "}") != string::npos);
}

string OEEmulation::parseXMLProperties(string value,
									   OEPropertiesMap &propertiesMap)
{
	int startIndex;
